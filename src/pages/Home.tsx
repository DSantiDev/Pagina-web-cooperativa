...
const navLinks = [
  ...
  { label: 'Oficina Virtual', href: 'https://coovitel.coop/oficina-virtual', external: true },
]

const creditProducts = [
  { name: 'Educación', icon: '🎓', desc: 'Pregrado, postgrado, técnico y educación continua.' },
  { name: 'Recreación', icon: '✈️', desc: 'Viajes y gastos totales de tus destinos favoritos.' },
  { name: 'Fidelización', icon: '⭐', desc: 'Tasas y plazos que mejoran con tu antigüedad.' },
  { name: 'Vehículo', icon: '🚗', desc: 'Nuevo, usado o bicicleta eléctrica a tu medida.' },
  { name: 'Libre Inversión', icon: '💼', desc: 'Financia cualquier proyecto sin restricciones.' },
  { name: 'CrediSalud', icon: '🏥', desc: 'Médico, dental, óptico y cirugías estéticas.' },
  { name: 'Vivienda', icon: '🏠', desc: 'Adquiere, construye o mejora tu hogar.' },
  { name: 'Crediseguros', icon: '🛡️', desc: 'Pólizas para proteger a tu familia y bienes.' },
  { name: 'Cupo Rotativo', icon: '🔄', desc: 'Cupo personal reutilizable siempre disponible.' },
  { name: 'Compra de Cartera', icon: '💳', desc: 'Unifica tus deudas con mejor tasa y plazo.' },
]

const savingsProducts = [
  { name: 'Cooviahorro', icon: '🎯', desc: 'Ahorro programado para construir el capital de tus sueños.' },
  { name: 'Cuenta de Ahorros', icon: '👁️', desc: 'Ahorro a la vista con disponibilidad y rentabilidad total.' },
  { name: 'Coovikids', icon: '🧒', desc: 'Ahorro infantil programado para el futuro de tus hijos.' },
  { name: 'Cooviducación', icon: '📚', desc: 'Ahorro dirigido a proyectos educativos propios o de tus hijos.' },
  { name: 'CDAT', icon: '📈', desc: 'Depósito a término con rentabilidad superior al mercado.' },
  { name: 'Coovinómina', icon: '💵', desc: 'Cuenta de nómina sin costos con todos los beneficios.' },
]
...
function ProductCard({ name, icon, desc, delay, visible, showLearnMore = true }: {
  name: string; icon: string; desc: string; delay: number; visible: boolean; showLearnMore?: boolean
}) {
  return (
    <div
      ...
      style={{
        ...
        transition: `opacity 0.7s cubic-bezier(0.16,1,0.3,1) ${delay}ms, transform 0.7s cubic-bezier(0.16,1,0.3,1) ${delay}ms, box-shadow 0.3s ease, border-color 0.3s ease`,
        willChange: 'opacity, transform',
      }}
      onMouseEnter={e => {
    ...
    >
      <div
        ...
        style={{ background: 'linear-gradient(135deg, rgba(200,184,255,0.08) 0%, rgba(27,42,143,0.04) 100%)' }}
      />
...
function SocialDock() {
  ...
  return (
    ...
    >
      {/* Toggle button — sits at bottom because column-reverse */}
      <button
        ...
        style={{
          ...
          boxShadow: open
            ? '0 0 0 3px rgba(232,160,32,0.55), 0 6px 20px rgba(27,42,143,0.4)'
      ...
      {/* Social links — rendered top→bottom in DOM but column-reverse flips them so WhatsApp is closest to button */}
      {socialLinks.map((s, i) => (
        <a
          key={s.label}
          href={s.href}
          ...
          style={{
            ...
            transition: `opacity 0.28s cubic-bezier(0.16,1,0.3,1) ${i * 45}ms, transform 0.28s cubic-bezier(0.16,1,0.3,1) ${i * 45}ms`,
            pointerEvents: open ? 'auto' : 'none',
            textDecoration: 'none',
          }}
          onMouseEnter={e => { (e.currentTarget as HTMLElement).style.filter = 'brightness(1.15) saturate(1.1)' }}
...
const heroBannerSlides = [
  {
    img: 'https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?w=700&h=875&fit=crop&auto=format',
    alt: 'Asesor financiero COOVITEL con asociado',
    caption: 'Bienvenido a COOVITEL',
    sub: 'Tu socio financiero de confianza',
    topLeft: { label: 'Asociados activos', value: '17,000+', dark: true },
    bottomRight: { label: 'Años de confianza', value: '64+', gold: true },
    midRight: { top: '✓ ISO 9001:2015', bottom: 'Bureau Veritas' },
    topRight: 'A+ Value & Risk',
  },
  {
    img: 'https://images.unsplash.com/photo-1521737604893-d14cc237f11d?w=700&h=875&fit=crop&auto=format',
    alt: 'Equipo COOVITEL',
    caption: 'Más de 200 empresas confían en nosotros',
    sub: 'Bienestar financiero para tus empleados',
    topLeft: { label: 'Empresas aliadas', value: '200+', dark: true },
    bottomRight: { label: 'Ciudades', value: '9', gold: true },
    midRight: { top: '✓ ISO 9001:2015', bottom: 'Bureau Veritas' },
    topRight: 'A+ Value & Risk',
  },
  {
    img: 'https://images.unsplash.com/photo-1529156069898-49953e39b3ac?w=700&h=875&fit=crop&auto=format',
    alt: 'Familia COOVITEL',
    caption: '64 años construyendo sueños',
    sub: 'Cooperativa Empresarial de Ahorro y Crédito',
    topLeft: { label: 'Años de trayectoria', value: '64+', dark: true },
    bottomRight: { label: 'Asociados activos', value: '17K+', gold: true },
    midRight: { top: '✓ ISO 9001:2015', bottom: 'Bureau Veritas' },
    topRight: 'A+ Value & Risk',
  },
]
...
function HeroBannerCarousel() {
  ...
  useEffect(() => {
    timerRef.current = setTimeout(() => {
      goTo((current + 1) % heroBannerSlides.length)
    }, 4500)
    ...
    <div className="hidden md:flex justify-center items-center">
      <div className="relative animate-float w-full max-w-[400px]">
        ...
        <div className="relative rounded-3xl overflow-hidden aspect-[4/5] shadow-2xl"
          ...
          />
          <div className="absolute inset-0" style={{ background: 'linear-gradient(to top, rgba(10,21,84,0.7) 0%, transparent 50%)' }} />
          <div className="absolute bottom-6 left-6 right-6"
            style={{ opacity: fading ? 0 : 1, transition: 'opacity 0.32s ease' }}>
            <p className="text-white font-bold">{s.caption}</p>
        ...
        >
          <p className="text-xs font-semibold text-white/70">{s.bottomRight.label}</p>
          <p className="font-black text-2xl text-white">{s.bottomRight.value}</p>
        </div>
...
export default function Home() {
  ...
  const productsSection = useInView(0.06)
  ...
  return (
    <div className="min-h-screen antialiased" style={{ fontFamily: "'Poppins', sans-serif", backgroundColor: '#f7f0ff' }}>
      ...
      {/* ── Navbar ─────────────────────────────────────────────── */}
      <nav className={`fixed top-[3px] inset-x-0 z-50 transition-all duration-400 ${
        scrolled ? 'bg-white/95 backdrop-blur-md shadow-sm border-b border-gray-100/80' : 'bg-transparent'
      }`}>
...
function ScrollToTop({ scrollPct }: { scrollPct: number }) {
  const visible = scrollPct > 8
  const SIZE = 52
  const STROKE = 3
  ...
  return (
    <button
      ...
      onMouseEnter={e => { e.currentTarget.style.background = 'rgba(27,42,143,0.85)' }}
      onMouseLeave={e => { e.currentTarget.style.background = 'rgba(10,21,84,0.55)' }}
    >
